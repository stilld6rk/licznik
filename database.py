...
def init_db():
    ...
    with engine.connect() as conn:
        migrations = [
            ...
            # Postgres has no "ADD CONSTRAINT IF NOT EXISTS" — guard it explicitly
            """DO $$ BEGIN
                 IF NOT EXISTS (
                   SELECT 1 FROM pg_constraint WHERE conname = 'uq_discord_guild_name'
                 ) THEN
                   ALTER TABLE guild_configs ADD CONSTRAINT uq_discord_guild_name UNIQUE (discord_guild_id, guild_name);
                 END IF;
               END $$""",
            ...
            # Payments are a fully global ledger keyed by nick — the per-guild member_id link is gone
            ...
            # Manual corrections are a global ledger keyed by nick too, same as payments
